...
# 1. Асинхронная обработка уведомлений через Django Signals + Celery
...
-   Django View обрабатывает запрос, вызывает метод модели lesson.complete_lesson().
...
-   Django немедленно возвращает ответ пользователю. GIL (Global Interpreter Lock) освобождается для обработки других запросов.
...
Вывод: Решение с сигналами является оптимальным балансом между надежностью, производительностью, простотой реализации и соответствием духу Django.

# 3. Инструкция по запуску и проверке
Как убедиться, что задачи отправляются
Запустите проект по инструкции выше.

Измените статус урока, например, через админку (/admin) или Swagger (/swagger), вызвав эндпоинт POST /api/v1/lessons/1/complete/.

Просмотрите логи:

Логи Django (веб-сервер):

```bash
docker logs -f lesson_app
```
Здесь вы увидите запись о вызове API и о том, что задача поставлена в очередь: "Уведомление поставлено в очередь.".

Логи Celery Worker:
```bash
docker logs -f lesson_celery_worker 
```
Здесь вы увидите процесс выполнения задачи: "[CELERY] Урок завершен: ...", затем паузу ~5 секунд и сообщение об успешной "отправке".
...
Проект задуман как основа, готовый к доработке:
